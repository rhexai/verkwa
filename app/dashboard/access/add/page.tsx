      ...
      .insert([
        {
          ...
          last_name: formData.last_name || "Staff",
          email: formData.email || `staff-${Date.now()}@verkwa.com`,
          role: formData.role,
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-2 text-[18px] font-black tracking-tight">
        <Link href="/dashboard/access" className="text-fuchsia-900 hover:underline">Staff</Link>
      ...
      <div className="bg-white border border-[#e5e7eb] shadow-sm rounded-xl overflow-hidden pb-12">
        ...
        <div className="p-6 border-b border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button 
              onClick={handleSave}
              disabled={loading}
              className="bg-black hover:bg-slate-800 text-white px-8 py-2.5 rounded text-[13px] font-bold uppercase tracking-wider flex items-center gap-2 shadow-lg shadow-black/10 transition-all active:scale-95 disabled:bg-slate-300"
            >
              {loading ? "..." : "SAVE"}
            </button>
          </div>
        ...
        <div className="p-8 max-w-4xl space-y-8 mt-4 font-plus-jakarta-sans">
          ...
          <div className="relative group">
            <label className="absolute -top-2.5 left-3 bg-white px-1 text-[13px] text-slate-500 font-bold group-focus-within:text-fuchsia-600">role</label>
            <select 
              value={formData.role}
          ...
          {/* Hidden functional fields needed for a valid employee record */}
          <div className="grid grid-cols-2 gap-6 pt-4 opacity-50">
             <div className="relative">
                <label className="absolute -top-2.5 left-3 bg-white px-1 text-[11px] text-slate-400 font-bold uppercase">First Name</label>
                <input 
                  type="text" 
                  value={formData.first_name}
                  onChange={(e) => setFormData({...formData, first_name: e.target.value})}
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-lg text-sm"
                  placeholder="Functional only"
                />
             </div>
             <div className="relative">
                <label className="absolute -top-2.5 left-3 bg-white px-1 text-[11px] text-slate-400 font-bold uppercase">Last Name</label>
                <input 
                  type="text" 
